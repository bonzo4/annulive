...
export default function RoadmapDisplay({
  ...
  try {
    ...
    if (!parsedContent.steps || !Array.isArray(parsedContent.steps)) {
      return (
        <RoadmapError
          title="Invalid Trunk Track Data"
          error="The response does not contain valid trunk track layers. Please try generating a new trunk track."
    ...
    return (
      <RoadmapError
        title="Invalid Trunk Track Data"
        error="The trunk track content could not be parsed. Please try generating a new trunk track."
        onCreateNew={onCreateNew}
        showTroubleshooting={false}
      />
    );
  ...
  return (
    <div className="mx-auto max-w-4xl rounded-2xl bg-white p-8 shadow-xl">
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <div className="mt-2 flex items-center gap-4">
            ...
            <span className="text-sm text-gray-500">
              Progress:{" "}
              <strong>
                {completedSteps.size}/{roadmapData.steps.length} layers
              </strong>
            </span>
        ...
        <Button onClick={onCreateNew}>Create New Trunk Track</Button>
      </div>
        ...
        <p className="text-sm text-amber-800">
          ...
          {progressPercentage === 100 && (
            <span className="mt-1 block font-semibold text-green-800">
              🎉 Congratulations! You&apos;ve completed your learning trunk track!
            </span>
          )}
        </p>
      </div> */}
